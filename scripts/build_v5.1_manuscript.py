#!/usr/bin/env python3
"""
V5.2 Manuscript Builder
Reads V5 docx → inserts figures/tables → appends supplementary materials → outputs V5.2_submission.docx
"""
import os
import re
import copy
...
OUT_DOCX = os.path.join(BASE, 'reports', 'manuscript_draft_v5.2_submission.docx')
...
MAIN_FIGURES = [
    # ---- Results 3.1: Pipeline ----
    {
        'keyword': 'discovery cohort of 164',
        'path': os.path.join(FIG_PNG_ACTIVE, 'Fig1_pipeline.png'),
        'width': 6.0,
    ...
    # ---- Results 3.2: SHAP ----
    {
        ...
        'path': None,  # skip — Fig2 goes after 3.2
        'width': 0,
        'caption': '',
    },
    {
        'keyword': 'Gene-level validation',
        'path': os.path.join(FIG_PNG_ACTIVE, 'Fig2_SHAP_beeswarm.png'),
        'width': 5.5,
    ...
    # ---- Results 3.3: Pathway Nomogram ----
    {
        'keyword': 'pathway-level modeling',
        'path': os.path.join(FIG_PNG_ACTIVE, 'Fig3A_pathway_volcano.png'),
        'width': 5.5,
        'caption': 'Figure 3. Pathway-level differential activity analysis. (A) Volcano plot of 44 ssGSEA pathway activities comparing resistant vs sensitive tumors in GSE39582 (n = 164). Dashed lines indicate |log2FC| > 0.1 and FDR < 0.10. (B) Consensus heatmap of top differentially active pathways across all five GEO cohorts (GSE39582, GSE104645, GSE28702, GSE72970, GSE69657). (C) Boxplots of the top four differentially active pathways in GSE39582.',
    },
    {
        'keyword': 'C-index of 0.676',
        'path': os.path.join(FIG_PNG_ACTIVE, 'Fig4A_PRS_ROC.png'),
        'width': 5.5,
        'caption': 'Figure 4. Pathway-level prognostic nomogram for XELOX-treated CRC. (A) LASSO-PRS ROC curves across GSE39582 training and four validation cohorts (GSE104645, GSE28702, GSE72970, GSE69657). (B) Kaplan-Meier curves for high-risk vs low-risk groups stratified by median LASSO-PRS in GSE39582 (n = 227, log-rank p < 0.001). (C) Forest plot of the 8-variable AIC-selected Cox regression model (GSE39582, 79 events, EPV = 9.9). (D) Nomogram for predicting 12/36/60-month RFS probability. (E) Bootstrap-based calibration curves at 12 and 36 months (B = 200, optimism-corrected). (F) Calibration curve at 60 months.',
    },
    # ---- Bootstrap results ----
    {
        'keyword': 'backward AIC model selection across',
        'path': None,  # supplementary figures
        'width': 0,
        'caption': '',
    },
    # ---- TGF-β ----
    ...
    # ---- Fair comparison ----
    {
        ...
        'path': os.path.join(FIG_PNG_ACTIVE, 'Fig6A_ablation_forest.png'),
        'width': 5.5,
        'caption': 'Figure 6. Fair comparison isolating feature type from modeling strategy. (A) Forest plot of the 23-gene AIC-Cox ablation model trained on GSE39582 (n = 227, 79 events). The model shows apparent C-index = 0.834 but severe overfitting (EPV = 3.4, optimism-corrected C = 0.406). (B) Four-configuration comparison: gene-level vs pathway-level features crossed with ensemble ML (XGBoost) vs AIC-Cox regression. Pathway-level AIC-Cox achieved the highest optimism-corrected C-index (0.489), demonstrating that pathway aggregation provides implicit regularization independent of the choice of regression method.',
    },
    # ---- Meta-analysis ----
    {
        'keyword': 'Stouffer weighted meta-analysis',
        'path': os.path.join(FIG_PNG_ACTIVE, 'Fig5_BP_dotplot.png'),
        'width': 5.5,
        'caption': 'Figure 5. Meta-analysis enrichment landscape. GO Biological Process and KEGG pathway enrichment for the 253-gene meta-analysis signature (Stouffer weighted, FDR < 0.05, directional consensus in >= 4/5 cohorts: GSE39582, GSE104645, GSE28702, GSE72970, GSE69657). Dotplot shows the top 10 enriched terms; barplot shows full enrichment results. Five convergent themes were identified: immune regulation, extracellular matrix remodeling, metabolism, cell cycle/DNA repair, and Wnt/TGF-beta signaling.',
    },
]
...
def main():
    ...
    # Fig 3B (pathway heatmap) and 3C (boxplot) — after Fig 3A
    for extra_fig, extra_cap in [
        (os.path.join(FIG_PNG_ACTIVE, 'Fig3B_pathway_heatmap.png'), ''),
        (os.path.join(FIG_PNG_ACTIVE, 'Fig3C_pathway_boxplot.png'), ''),
    ]:
        if os.path.exists(extra_fig):
            for i, para in enumerate(doc.paragraphs):
                if 'Figure 3.' in para.text and 'volcano' in para.text.lower():
                    add_figure_safe(doc, para, extra_fig, 5.5, extra_cap)
                    break
    
    # Fig 4B-F panels — after Fig 4A
    fig4_panels = [
        (os.path.join(FIG_PNG_ACTIVE, 'Fig4B_prs_km_curve.png'), ''),
        (os.path.join(FIG_PNG_ACTIVE, 'Fig4C_cox_forest.png'), ''),
        (os.path.join(FIG_PNG_ACTIVE, 'Fig4D_nomogram.png'), ''),
        (os.path.join(FIG_PNG_ACTIVE, 'Fig4E_calibration_12mo.png'), ''),
        (os.path.join(FIG_PNG_ACTIVE, 'Fig4E_calibration_36mo.png'), ''),
        (os.path.join(FIG_PNG_ACTIVE, 'Fig4F_calibration_60mo.png'), ''),
    ]
    for extra_fig, extra_cap in fig4_panels:
        if os.path.exists(extra_fig):
            for i, para in enumerate(doc.paragraphs):
                if 'Figure 4.' in para.text and 'ROC' in para.text:
                    add_figure_safe(doc, para, extra_fig, 5.5, extra_cap)
                    break
    
    # Fig 5 panels — KEGG barplot + high-tier
    fig5_panels = [
        (os.path.join(FIG_PNG_ACTIVE, 'Fig5_BP_barplot.png'), ''),
        (os.path.join(FIG_PNG_ACTIVE, 'Fig5_KEGG_barplot.png'), ''),
        (os.path.join(FIG_PNG_ACTIVE, 'Fig5_KEGG_high.png'), ''),
    ]
    for extra_fig, extra_cap in fig5_panels:
        if os.path.exists(extra_fig):
            for i, para in enumerate(doc.paragraphs):
                if 'Figure 5.' in para.text and 'dotplot' in para.text.lower():
                    add_figure_safe(doc, para, extra_fig, 5.5, extra_cap)
                    break
    
    # Fig 6B — after Fig 6A
    fig6b = os.path.join(FIG_PNG_ACTIVE, 'Fig6B_method_comparison.png')
    if os.path.exists(fig6b):
        for i, para in enumerate(doc.paragraphs):
            if 'Figure 6.' in para.text and 'ablation' in para.text.lower():
                add_figure_safe(doc, para, fig6b, 5.5, '')
                break
    ...
    # Fix "bootstrap-corrected 0.692" → "bootstrap mean C-index = 0.692" (ALL occurrences)
    fix_count = 0
    for para in doc.paragraphs:
        for run in para.runs:
            if 'bootstrap-corrected 0.692' in run.text:
                ...
                fix_count += 1
    print(f'  [OK] Fixed Results text: {fix_count} occurrence(s)')
    ...
    ref_fixes = {
        ...
        '[Authors]. Serum metabolomics reveals biomarkers for XELOX chemotherapy efficacy prediction in colorectal cancer. Sci Rep. 2025;15:8942.':
            'Sun J, et al. Serum metabolomics reveals biomarkers for XELOX chemotherapy efficacy prediction in colorectal cancer. Sci Rep. 2025;15:8942.',
    ...
    print(f'  Total reference fixes: {fix_count}')
    ...
    if len(doc.tables) >= 2:
        ...
        add_table_footnotes(doc, doc.tables[0],
            ...
            'Mean |SHAP| from XGBoost classifier. '
            'p-values for differential expression (R vs S, GSE39582 n = 164) are provided in Supplementary Table S2.')
        
        # Add footnotes for Table 2 (Nomogram) — fix optimism and note ΔC column
        add_table_footnotes(doc, doc.tables[1],
            'Note: HR = hazard ratio; CI = confidence interval. '
            'Apparent C-index = 0.676; bootstrap mean C-index = 0.692 '
            '(B = 1,000, 95% CI 0.632\u20130.751). '
            'EPV = events per variable = 9.9 (79 events / 8 variables). '
            'All pathway scores derived from ssGSEA of 44 KEGG + Hallmark gene sets. '
            'Tumor location: distal vs proximal (reference). '
            '\u0394C if removed: leave-one-variable-out analysis was not performed; '
    ...
    summary_p.add_run('Version: V5.2 (Submission-ready)\n')
