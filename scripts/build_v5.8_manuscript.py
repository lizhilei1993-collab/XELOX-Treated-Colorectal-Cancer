...
SRC_DOCX = os.path.join(BASE, 'reports', 'manuscript_draft_v5.1_submission.docx')
OUT_DOCX = os.path.join(BASE, 'reports', 'manuscript_draft_v5.8_submission.docx')
...
MAIN_FIGURES = [
    ...
    },
    {
        ...
        'path': fig_path('Fig3EFG_calibration_improved.png'),
        'width': MULTI_PANEL_WIDTH,
        'caption': 'Figure 4 (continued). Bootstrap-based calibration curves for the 8-variable Cox nomogram with ideal calibration line (45-degree diagonal). (E) 12-month calibration (slope = 0.817, Brier = 0.142). (F) 36-month calibration (slope = 0.817, Brier = 0.201). (G) 60-month calibration (slope = 0.817, Brier = 0.218). B = 200 bootstrap resamples, optimism-corrected.',
    },
    # ---- TGF-β ----
    {
        'keyword': 'TGF-β signaling paradox',
...
def main():
    ...
    # Variable instability discussion
    instability_text = (
        'Variable stability assessment through Ridge and Elastic Net penalization revealed '
        'important heterogeneity among the 8 nomogram variables (Supplementary Tables S34-S35). '
        'Four variables demonstrated robust stability: HALLMARK_WNT_BETA_CATENIN_SIGNALING '
        '(Elastic Net selection frequency = 92%, Ridge HR = 1.18), KEGG_ECM_RECEPTOR_INTERACTION '
        '(100%, HR = 1.09), HALLMARK_MYC_TARGETS_V2 (84%, HR = 0.92), and location_distal '
        '(88%, HR = 1.28). However, two variables showed concerning instability: KEGG_PATHWAYS_IN_CANCER '
        '(selection frequency = 32%, Ridge HR = 0.995, effectively null) and KEGG_COLORECTAL_CANCER '
        '(28%, HR = 0.999). These findings suggest that these two variables may be noise-driven '
        'selections in the unpenalized Cox model, and their inclusion in the final nomogram should be '
        'interpreted with caution. Notably, the TGF-\u03b2 paradox (opposing HR directions for HALLMARK '
        'vs KEGG gene sets, r = 0.874) persists under Ridge penalization, supporting its biological '
        'validity rather than treating it as a statistical artifact of multicollinearity (VIF = 932 '
        'and 407, respectively). The ridge-penalized model provides a more conservative effect '
        'estimation that down-weights these unstable variables while preserving the robust core '
        'of the nomogram.'
    )
    ...
    for i, para in enumerate(doc.paragraphs):
        if 'Important limitations of this study must be acknowledged' in para.text:
            ...
            inst = doc.add_paragraph(instability_text)
            para._element.addnext(inst._element)
            para._element.addnext(cap._element)
            print('  [OK] Inserted enhanced overfitting + instability discussion')
            break
    else:
