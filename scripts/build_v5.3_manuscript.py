...
OUT_DOCX = os.path.join(BASE, 'reports', 'manuscript_draft_v5.3_submission.docx')
...
def main():
    ...
    # Phase C: update in-text citations from (20) to (30)
    cite_fixes = 0
    for para in doc.paragraphs:
        for run in para.runs:
            # Match standalone citation to ref 20 (not 200, 120, etc.)
            text = run.text
            new_text = re.sub(r'\((20)\)', r'(30)', text)
            new_text = re.sub(r'\[20\]', '[30]', new_text)
            if new_text != text:
                run.text = new_text
                cite_fixes += 1
    ...
    if len(doc.tables) >= 2:
        ...
        # Add footnotes for Table 2 (Nomogram) — corrected bootstrap description
